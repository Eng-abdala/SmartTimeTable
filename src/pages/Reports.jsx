...
export function Reports() {
  ...
  const selectedSubjectName = searchParams.get('subject')
  ...
  const rows = useMemo(() => {
    ...
    subjects.forEach(subject => {
      ...
      if (!grouped.has(key)) {
        grouped.set(key, { name: subject.name.trim(), codes: new Set(), semesterIds: new Set(), totalHours: new Set(), theoryHours: new Set(), labHours: new Set(), lecturerIds: new Set(), lecturersList: new Map() })
      }
      ...
      getSubjectLecturers(subject.id).forEach(lecturer => {
        entry.lecturerIds.add(lecturer.id)
        entry.lecturersList.set(lecturer.id, lecturer)
      })
    })
    return Array.from(grouped.values()).map(entry => ({
      ...
      lecturers: Array.from(entry.lecturersList.values()).sort((a, b) => a.name.localeCompare(b.name)),
    })).filter(subject => {
  ...
  const selectedSubject = useMemo(() => {
    ...
    return rows.find(r => r.name === selectedSubjectName) || null;
  }, [rows, selectedSubjectName]);

  const setSelectedSubject = (subject) => {
    if (subject) {
      setSearchParams({ subject: subject.name })
    } else {
      setSearchParams({})
    }
  }
  ...
  if (selectedSubject) {
    return (
      <section>
        ...
        >
          <Icon name="back" className="h-4 w-4" /> Back to Subject List
        </button>
        <div className="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div className="bg-slate-50/80 p-6 sm:p-8 border-b border-slate-100">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-brand-600">{selectedSubject.code || 'No Code'}</p>
                <h1 className="mt-1 text-2xl font-bold text-brand-950">{selectedSubject.name}</h1>
              </div>
            </div>
          </div>
          <div className="p-0">
            {selectedSubject.lecturers && selectedSubject.lecturers.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead className="bg-brand-950 text-xs uppercase tracking-wide text-cyan-100">
                    <tr>
                      <th className="px-5 py-4">#</th>
                      <th className="px-5 py-4">Lecturer ID</th>
                      <th className="px-5 py-4">Full Name</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {selectedSubject.lecturers.map((l, i) => (
                      <tr key={l.id} className="transition hover:bg-brand-50/40">
                        <td className="px-5 py-4 text-slate-400 font-medium">{i + 1}</td>
                        <td className="px-5 py-4"><span className="rounded-lg bg-slate-100 px-2 py-1 font-mono text-xs font-bold text-slate-600">{l.lecturer_id || '—'}</span></td>
               ...
               <p className="text-slate-500 p-6 text-sm">No teachers assigned to this subject.</p>
            )}
          </div>
        </div>
      </section>
